...
const cityList = cities.filter((city) => city.charAt(city.length - 1) === 'i'
);
...
//   } else {
//     return console.log(`The fruit with index ${fruits.indexOf(item)}} is neither apple or orange`)
//    }
...
//Jareds way of solving this problem

fruits.forEach((fruitObject, index) => {
  if (fruitObject.color === 'red') {
    console.log(`The fruit with the index ${index} is an apple.`)
  } else if (fruitObject.color === 'orange') {
    console.log(`The fruit with index ${index} is an orange.`)
  } else {
    console.log(`Thefruit with index ${index} is neither apple nor orange`)
  }
})
